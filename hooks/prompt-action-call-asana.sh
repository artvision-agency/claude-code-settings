...
2. **Если задача реальна** — создать в Asana по `asana-required-fields.md`:
   ...
   - Если хватает контекста (упомянут клиент, дедлайн, исполнитель явно или из default'ов) — создать молча, отчёт одной строкой: «Asana: <name> → <assignee>, due <date>, GID <gid>».
