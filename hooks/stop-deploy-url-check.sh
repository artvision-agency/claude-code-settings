...
# Транскрипта нет, хотя session_id получен → НЕ можем проверить.
# Для Stop-хука блокировать завершение здесь бесполезно (деплой, если был, уже прошёл —
# exit 2 ничего не защитит, только помешает закрыть сессию). НЕ молчим (no-smoothing):
# видимый warn в stderr, но exit 0 (не блокируем Stop). Это мягкий fail-CLOSED именно для
# случая «нет транскрипта»; для malformed-stdin/нет-python3/нет-session_id остаётся exit 2.
if [[ ! -f "$JSONL" ]]; then
    printf '⚠️  deploy-url-check: транскрипт сессии не найден (%s) — проверку «URL первой строкой» выполнить не удалось. Если был deploy, проверь вручную.\n' "$JSONL" >&2
    exit 0
fi
