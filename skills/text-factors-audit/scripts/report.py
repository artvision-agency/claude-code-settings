#!/usr/bin/env python3
"""
Text Audit Report — JSON от audit.py → HTML в стиле CAMEO.

CLI:
    python3 report.py text-audit.json --output=report.html --client-name="Example"
"""
...
CATEGORY_LABELS: dict[str, str] = {
    "meta_tags": "Мета-теги и сниппет",
    "structure": "Структура и заголовки",
    "volume": "Объём и длина текста",
    "density": "Плотность ключей, тошнота, водность",
    "semantics": "LSI и семантическая полнота",
    "uniqueness": "Уникальность и дубли",
    "readability": "Читаемость текста",
    "antispam": "Антиспам",
    "images": "Изображения и alt-тексты",
    "links": "Ссылки в тексте",
    "freshness": "Свежесть контента",
    "commercial_triggers": "Коммерческие триггеры",
}
...
def render_report(
    ...
    for cat_id in CATEGORY_ORDER:
        ...
        category_stats[cat_id] = {
            ...
            if stats
            else len(factors_by_category.get(cat_id, [])),
        }
    ...
    high_count = sum(
        1 for f in factors if f["status"] == "fail" and f["severity"] == "high"
    )
    ...
    context = {
        ...
        "keyword": audit_data.get("keyword") or "",
        "lsi_words": audit_data.get("lsi_words") or [],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Генератор HTML-отчёта текстового аудита"
    )
    ...
    parser.add_argument(
        "--client-name", default="Сайт клиента", help="Название клиента"
    )
