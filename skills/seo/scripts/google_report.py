...
# Lazy import: weasyprint only needed for PDF output
HTML = None
def _load_weasyprint():
    ...
    if HTML is None:
        try:
            ...
            print(f"Error: weasyprint unavailable ({e}). Use --format html.", file=sys.stderr)
            sys.exit(1)
    return HTML
...
def generate_report(report_type, data, domain, output_dir, output_format="pdf"):
    ...
    if output_format in ("pdf", "both", "all"):
        ...
        try:
            _load_weasyprint()(string=html_content).write_pdf(str(pdf_path))
