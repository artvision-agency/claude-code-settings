...
## 10. Архитектура — что делать с дня 1 (уроки Card Duel)

### База данных
- **PostgreSQL с миграциями** с первого дня. SQLite на Docker теряет WAL при restart
- Миграции — numbered SQL files, применяются автоматически при старте

### Серверная экономика
- **Все монеты, инвентарь, бустеры — ТОЛЬКО на сервере.** Клиент = UI only
- `coin_transactions` — таблица аудита каждой операции (зачисление, списание, причина)
- Если монеты на клиенте — читы неизбежны (LocalStorage легко редактируется)

### WebSocket фреймворк
- **Один WS фреймворк** — не параллелить Legacy WS + Colyseus. Выбрать один сразу
- Миграция с одного на другой — крайне болезненна (переписывание всей сетевой логики)

### Shared engine kernel
- `packages/shared/` — damage, combo, balance. **Zero дублирование** клиент-сервер
- Если расчёт есть на клиенте и на сервере — он ДОЛЖЕН быть в shared

### Валидация и логирование
- **Zod валидация** на все входные данные сервера (никаких `as any` на external data)
- **Remote client logger** — warn/error с клиента отправляются на сервер для отладки
- `yandexId` и другие внешние ID — regex whitelist, не доверять сырым данным

---

## 11. Монетизация — уроки

### Антипаттерны (что НЕ делать)
| Ошибка | Последствие | Решение |
|--------|-------------|---------|
| Монеты на клиенте | Читы через DevTools | Серверная экономика + `coin_transactions` аудит |
| Бустеры/герои на клиенте | Бесплатная активация через localStorage | `inventory-repo` + `PRICE_LIST` на сервере |
| Тестовые цены в проде | Все items по 1 coin | `DEV` flag для тестовых цен, prod = реальные |

### Модель монетизации
- **Buy-once-use-once** для бустеров: покупка в магазине → инвентарь → списание при использовании
- **Monetization gating**: `isMonetized = isAuthenticated`. Гости играют без монет/магазина
- **PRICE_LIST** — единый объект цен, sync-тест клиент vs сервер обязателен

### Retention механики
- **Daily rewards** — streak 7 дней (1→2→3→5→8→13→21 coins)
- **Daily quests** — 5 квестов/день (win 3, play 5, use combo, deal damage, collect pairs)
- Награды начисляются только через серверный endpoint

---

## 12. Платформы — порядок публикации (обновлённый)

### Рекомендуемая последовательность
1. **itch.io первым** — нет модерации, 10 минут, мгновенный фидбек от игроков
2. **Yandex Games** — низкий порог модерации, native auth + монетизация, большая RU-аудитория
3. **CrazyGames** — требует: туториал за 30 секунд, SFX обязательно, onboarding flow
4. **Poki** — 2% acceptance rate, долгий ответ (1-2 недели), строжайшие требования к качеству
5. **VK/Telegram** — нужен собственный домен + SSL

### Платформенные требования (детали)

**Yandex Games:**
- SDK init → `GameplayAPI.start()` → `LoadingAPI.ready()`
- `pause/resume` events — мьютить звук, ставить на паузу
- Leaderboards — создать в консоли ДО загрузки билда
- Cloud saves через `player.setData()`

**CrazyGames:**
- SDK init + sitelock (домен проверка) обязательны
- `GameplayAPI.start()/stop()` для трекинга сессий
- Ads: interstitial + rewarded (обязательны для монетизации)
- User auth через `getUserToken()`

**Все платформы:**
- `VITE_PLATFORM` env переменная для каждого билда
- Отдельные build scripts: `VITE_PLATFORM=<name> npx vite build --outDir dist-<name>`
- ZIP < 5MB рекомендуется (< 200MB лимит)

---

## 13. Ошибки которые нельзя повторять

### Навигация и серверные подтверждения
- **Premature navigation** — НИКОГДА не переключать экран до получения server confirmation
- Пример: клиент нажал "Ready" → ждём WS `game_started` → только тогда показываем игровой экран

### Побочные эффекты в shared методах
- Метод `queue.remove()` удалял `pendingNames` — побочный эффект ломал другие вызовы
- **Правило:** если метод shared — трассировать ВСЕ вызывающие места перед изменением

### Типизация и валидация
- `as any` на external data без runtime validation = уязвимость
- **Всегда:** Zod schema или regex whitelist для внешних данных
- `yandexId` — `/^[a-zA-Z0-9_-]{1,64}$/` regex whitelist

### Утечки памяти
- **Uncleaned timers** при выходе из игры — setInterval/setTimeout без clearInterval
- **Правило:** каждый setInterval → сохранять id → очищать в onUnmounted/cleanup

### Клиент-серверная симметрия
- Combo/damage логика различалась на клиенте и сервере → desync
- **Решение:** вся логика в `packages/shared/`, один источник правды

### CSS ловушки
- `nth-child` в parent побеждает child `z-index` — использовать `:deep()` в scoped styles
- `touch-action: manipulation` на `*` ломает input/select — применять точечно
...
### Технологии
- **Howler.js** (не Web Audio API — слишком сложно для игровых звуков)
- Budget: ~500-750KB total на все звуки

### Архитектура SFX
- SFX layers: файл + volume + rate + delay
- Каждое действие может иметь несколько слоёв звука (атака = удар + реакция + UI-клик)

### Музыка
- Crossfade 800ms между треками
- Случайный выбор из пула (не один и тот же трек по кругу)

### Lifecycle
- **Preload** при первом клике пользователя (user gesture required для Web Audio)
- **Mute** при pause/ads (платформа требует тишину)
- **Unmute** при resume
- На мобильных — дополнительная проверка user gesture

---

## 15. Баланс героев и игровой экономики

### Уроки балансировки (Card Duel)
| Герой/механика | Проблема | Решение |
|----------------|----------|---------|
| Лучник (+3 flat dmg) | Слишком слабый для одного срабатывания | Увеличить бонус или дать multiple triggers |
| Берсерк (+25% при low HP) | Потенциально OP | Проверить HP sync, playtesting |
| Тень (10% dodge) | Frustrирующий RNG для противника | Снизить до 8%, визуальный фидбек |
| Жрица (800 coins) | Переоценена, никто не покупает | Снизить цену до 500-600 |
| +15% урон на картах 2-3 | Не ощущается (Math.round съедает) | Минимальный бонус = 1 damage |

### Общие правила баланса
- Flat бонусы лучше % бонусов на малых числах (Math.round делает % незаметным)
- RNG-механики (dodge, crit) — не выше 10%, иначе фрустрация
- Цены тестировать на реальных игроках, не на dev-аккаунтах

---

## 16. Тестирование (расширенное)

### TDD цикл
1. **RED** — написать тест, убедиться что он падает
2. **GREEN** — минимальный код чтобы тест прошёл
3. **REFACTOR** — улучшить код, тесты всё ещё зелёные

### Обязательные моки
- `vi.mock('fetch')` для серверных API вызовов
- `vi.mock('logger')` во всех тестах (чтобы не спамить в консоль)
- `vi.clearAllMocks()` в каждом `beforeEach`

### Критические тесты
- **PRICE_LIST sync** — клиентский и серверный PRICE_LIST идентичны
- **Integration тесты** для PvP flow (подключение → матчмейкинг → игра → результат)
- **Edge cases:** отключение во время игры, двойной клик, race conditions

---

## 17. Техдолг — как избегать

### Антипаттерны из Card Duel
| Проблема | Масштаб | Как не допустить |
|----------|---------|------------------|
| `game.ts` 2070 строк | Невозможно читать/рефакторить | Разбивать на Domain + Application слои с дня 1 |
| `index.ts` 544 строк if-chain | Маршрутизация как лапша | Router pattern (map action → handler) |
| Сырой SQL везде | Дублирование, SQL injection | Repository pattern с первого запроса |
| 62 использования `any` | Нет type safety | `strict: true` в tsconfig, zero tolerance к `any` |
| TODO/FIXME в коде | Копятся, забываются | Трекать в issues/задачах, не в комментариях |

### Правило 200 строк
- Файл > 200 строк — повод для разбиения
- Функция > 30 строк — повод для декомпозиции
- Модуль с > 5 зависимостями — повод для слоя абстракции

---

## 18. QA чеклист перед публикацией (полный)

### Обязательно для всех платформ
- [ ] Туториал проходится за 30 секунд
- [ ] Все SFX + BGM работают (не заглушки)
- [ ] Mobile responsive: 600px → 2160px (portrait + landscape)
- [ ] Offline mode или graceful error при потере соединения
- [ ] Server-side economy (нет DevTools cheating)
- [ ] Performance: 60 FPS, < 5MB initial load
- [ ] i18n: English + Russian minimum
- [ ] Platform SDK integration verified (auth, ads, saves)
- [ ] Remote error logging работает и отправляет на сервер
- [ ] PRICE_LIST sync тест проходит (клиент = сервер)
- [ ] Нет `console.log` спама (только через logger)
- [ ] Все таймеры очищаются при выходе из экрана
- [ ] Touch targets ≥ 44px
- [ ] `overflow: hidden` на html/body
- [ ] ZIP собирается и содержит все assets
