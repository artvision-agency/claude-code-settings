...
# зоны
# KMV_CITY — УЗКИЙ список ГОРОДОВ-курортов КМВ. Только их явное упоминание
# даёт 🔴 (вариант «б»): отсекает шум частых ОБЩЕКРАЕВЫХ объявлений губернатора.
...
def classify(ch, text):
    ...
    Общекраевые объявления губернатора (без города) и маршрут НЕ пушатся
    мгновенно — они всплывут в дайджесте 08:30 как 🟡. Так real-time почти
    всегда «тихо» и срабатывает лишь на реальный КМВ-город.
    ...
    if not has(t, PRIMARY):        # нет триггерного слова
        return None, None
    # 🔴 ТОЛЬКО при явном городе-курорте КМВ (не «Ставропольского края» вообще,
    #    не «Невинномысск»/«Ставрополь» — это краевое/маршрут, см. KMV_CITY).
    if has(t, KMV_CITY):
        return "kmv", "red"
