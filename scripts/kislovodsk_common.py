...
# ── Конфиг (каналы / слова / coords / пороги вынесены в JSON) ────────────────
# Правка источников/слов/порогов = редактировать kislovodsk-config.json, без
# кода и без LLM. Файла нет / битый → используются inline-дефолты ниже (скрипт
# НЕ падает). Путь переопределяется env KMV_CONFIG_JSON (portable Mac↔VPS).
CONFIG_PATH = os.environ.get('KMV_CONFIG_JSON') or os.path.join(
    os.path.dirname(os.path.abspath(__file__)), 'kislovodsk-config.json')
...
def _load_config(path=CONFIG_PATH):
    ...
    except Exception:          # нет файла / битый JSON → пустой (фолбэк на дефолты)
        return {}
...
def cfg_get(dotted, default=None):
    """Значение по dotted-пути ('thresholds.realtime.near_window') или default."""
    cur = _CFG
...
def cfg_num(dotted, default):
    """Число из конфига (int/float, не bool) или default (type-safe)."""
    v = cfg_get(dotted)
...
def cfg_list(dotted, default):
    """Список строк из конфига (lower-case, непустой) или default."""
    v = cfg_get(dotted)
...
def cfg_pairs(dotted, default):
    """Список пар [kw,key] из конфига (kw→lower) или default."""
    v = cfg_get(dotted)
    if isinstance(v, list) and v:
        out = []
        for it in v:
            if isinstance(it, (list, tuple)) and len(it) == 2:
                out.append((str(it[0]).lower(), str(it[1])))
        if out:
            return out
    return default


def cfg_coords(default):
    """{город: (lat, lon)} из конфига или default."""
    v = cfg_get("coords")
    if isinstance(v, dict) and v:
        ...
        for k, pair in v.items():
            if isinstance(pair, (list, tuple)) and len(pair) == 2:
                try:
                    out[k] = (float(pair[0]), float(pair[1]))
...
def cfg_channels(dotted, default):
    """Список каналов [{u,zone,name,...}] из конфига (валидные u/zone/name) или default."""
    v = cfg_get(dotted)
    if isinstance(v, list) and v:
        out = [c for c in v if isinstance(c, dict) and c.get("u") and c.get("zone")]
        if out:
            return out
    return default


CONNECT_TIMEOUT = cfg_num("thresholds.connect_timeout", 25)   # сек на connect/is_user_authorized

# ── Словари триггеров (lower-case; дефолты ниже, override из config) ─────────
...
DRONE = cfg_list("keywords.drone", [
    "бпла", "беспилотник", "беспилотн", "шахед", "герань", "geran",
    "fpv", "ударный бпла", "дрон-камикадзе", "вражеский дрон"])
# AIRRAID — объявления воздушной/ракетной/беспилотной опасности.
AIRRAID = cfg_list("keywords.airraid", [
    "воздушная тревога", "план ковёр", "план ковер", "беспилотная опасность",
...
# PRIMARY — любой триггерный сигнал (БПЛА ИЛИ воздушная тревога). Производное.
PRIMARY = DRONE + AIRRAID
# IMPACT — НЕ триггер сам по себе (нужен PRIMARY рядом), повышает важность.
IMPACT = cfg_list("keywords.impact", [
    "сбит", "сбили", "сбито", "перехвач", "обломк", "прилёт", "прилет",
    "пострадав", "погиб", "разрушен", "возгорание", "пожар на"])
# FOREIGN — НЕ-КМВ юг (Сочи/Адлер/Краснодар/Крым…). Для real-time анти-🔴:
# если триггер «про» чужой город/аэропорт ближе, чем КМВ-город — не пушим
# мгновенно (роундап «аэропорт Сочи» из КМВ-канала ≠ угроза курорту КМВ).
FOREIGN = cfg_list("keywords.foreign", [
    "сочи", "адлер", "геленджик", "анапа", "новороссийск", "туапсе",
    "краснодар", "крым", "симферопол", "севастопол"])
...
KMV_CITY = cfg_list("keywords.kmv_city", [
    "кисловодск", "минеральн", "минвод", "ессентук",
    "пятигорск", "железноводск"])
# KMV_LOC — широкая КМВ/краевая зона (для зонирования both-каналов в дайджесте).
KMV_LOC = cfg_list("keywords.kmv_loc", [
    "кисловодск", "минеральные воды", "минводы", "минвод", "ессентуки",
    "пятигорск", "ставрополь", "кмв", "георгиевск", "невинномыс",
    "будённовск", "буденновск", "ставрополье", "ставропольск"])
ROUTE_LOC = cfg_list("keywords.route_loc", [
    "ростов", "тихорец", "невинномыс", "миллерово", "сулин", "батайск",
    "каменск", "лихая", "ростовск", "краснодарск", "кубан", "аксай",
    "шахт", "новочеркасск"])
# транспорт ж/д/аэропорт — для маршрута (дайджест)
TRANSPORT = cfg_list("keywords.transport", [
    "поезд", "поезда", "поездов", "ж/д", "жд ", "железнодорож",
    "электричк", "вокзал", "движение поездов", "состав"])
# сбой/последствие — для маршрута и красного уровня (дайджест)
DISRUPT = cfg_list("keywords.disrupt", [
    "задержк", "задержив", "отмен", "перекры", "приостановлен",
...
def min_dist(text, words_a, words_b):
    """Минимальное расстояние (в символах) между ближайшими вхождениями слова из
    words_a и слова из words_b. None, если хоть одна группа не встретилась."""
    t = (text or "").lower()
...
def near(text, words_a, words_b, window):
    ...
    d = min_dist(text, words_a, words_b)
...
PLACE_KEYS = cfg_pairs("place_keys", [
