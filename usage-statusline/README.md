...
```
stan@host | Opus 4.8 | my-home | master* | ctx:5% | $1.06 | me:4% | 5h:9% (12:49) wk:2% (Jun26 20:59)
```

- **`$N`** — стоимость текущей сессии (от Claude Code).
- **`me:N%`** — мой $-расход ЭТОЙ машины за неделю как % от своего лимита `weekly_limit_usd` (в `~/.claude/account-usage.json`; считается локально по суммам сессий, окно привязано к плановому сбросу). Только эта машина.
...
- В скобках у `5h`/`wk` — время сброса окна (сегодня `12:49`, иначе `Jun26 20:59`).
