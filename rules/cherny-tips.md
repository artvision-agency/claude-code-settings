...
## 4. Auto-compact — НЕ трогаем (решение Антона 11.06.2026)

> Было: совет Cherny `CLAUDE_CODE_AUTO_COMPACT_WINDOW=400000` — **не работал** (значение режется до реального окна 200K, проверено по env-vars docs). Удалён из settings.
> Пробовали: `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=50` (компакт на 50%). **Откачено в тот же день** — Антон: «авто-компакт = тот же /compact, обрезаются углы, мне это не надо».

**Решение Антона:** компакт = lossy-суммаризация, для долгих рабочих сессий НЕ использовать ранний авто-компакт. Вместо него — **handover-флоу**: при ~40-50% контекста (warning-хук уже инжектит) → skill `handover` (полный HANDOVER.md в проект) → новая сессия читает context-log+STATUS+handover (правило `resume-read-project-state.md`). Авто-компакт остаётся на дефолте ~95% только как страховка от упирания в лимит. Нативного «авто-создать новую сессию с передачей» в Claude Code нет (проверено 11.06).
