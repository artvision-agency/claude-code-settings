...
- **#34** Повторил ЧИСЛО из вторичного источника (ТГ-пост @agents_lab/124: «OpenRouter MCP = 9 tools») без сверки с первоисточником → офиц.дока openrouter.ai/docs/mcp-server: **13 tools**. Класс `numbers-deterministic-meaning-llm` + `service-knowledge-base` (офиц.справка = источник правды). Правило: ЛЮБОЕ число/спеку/лимит из поста/чата/памяти — сверять с первоисточником (вендор-дока) ДО утверждения пользователю; вторичка = UNCONFIRMED пока не сверено. Прецедент 2026-06-26. Поймал сам через /factcheck (хорошо), но «9» уже ушло Антону до сверки (плохо). Кандидат-хук: `stop-secondary-number-unverified.sh` (Stop, warn) — детект чисел про внешний сервис в ответе без признака WebFetch офиц.доки в turn. Bypass `SECONDARY_NUM_OK=1`. ⏳ approve.
